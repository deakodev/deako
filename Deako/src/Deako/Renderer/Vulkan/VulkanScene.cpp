...
namespace Deako {
    ...
    VulkanScene::VulkanScene(Scene* scene)
        : m_ParentScene(scene)
    {
    }
    ...
    {
        VulkanSceneResources& vs = m_Resources;
    ...
    void VulkanScene::SetUpAssets()
    {
        ...
        for (auto it = m_ParentScene->GetEntities().begin(); it != m_ParentScene->GetEntities().end(); )
        {
            Entity entity = *it;
            ...
            if (tag == "Skybox")
            {
                ...
                vs.skybox.model = model; // easy access, separate from others
                m_ParentScene->GetEntities().erase(it); // erase the entity and update the iterator

                // Handle the skybox-specific components
                auto& textureComp = entity.GetComponent<TextureComponent>();
                vs.skybox.environmentCube = assetPool.GetAsset<TextureCubeMap>(textureComp.handle);
            ...
            {
                m_ParentScene->GetEntities().erase(it);
                continue;
            }
    ...
    void VulkanScene::SetUpUniforms()
    {
        ...
        size_t dynamicBufferSize = m_ParentScene->GetEntities().size() * vs.dynamicUniformAlignment;
    ...
    void VulkanScene::SetUpDescriptors()
    {
        ...
        {   // static descriptor pool allocator
            ...
            for (auto& entity : m_ParentScene->GetEntities())
            {
                if (entity.HasComponent<PrefabComponent>())
                {
                    auto& prefabComp = entity.GetComponent<PrefabComponent>();
        ...
        {   // model node matrices
            ...
            auto AllocateNodeDescriptorSet = [&vs](Node* node)
                {
    ...
    void VulkanScene::OnUpdate()
    {
        ...
        Ref<EditorCamera> camera = m_ParentScene->GetActiveCamera();
        // shared scene
        ...
        memcpy(uniformSet.dynamic.buffer.mapped, vs.uniformDynamicData.model, vs.dynamicUniformAlignment * m_ParentScene->GetEntities().size());
        ...
        memoryRange.size = vs.dynamicUniformAlignment * m_ParentScene->GetEntities().size();
        vkFlushMappedMemoryRanges(vb.device, 1, &memoryRange);

        VulkanPicker::OnUpdate();
    }
    ...
    void VulkanScene::Draw(VkCommandBuffer commandBuffer, DkU32 imageIndex)
    {
        VulkanSceneResources& vs = m_Resources;
        VulkanBaseResources& vb = VulkanBase::GetResources();
        RendererStats& stats = Renderer::GetSceneStats();
        ...
        for (auto& entity : m_ParentScene->GetEntities())
        {
            if (entity.HasComponent<PrefabComponent>())
            {
                ...
                vs.boundPipeline = VK_NULL_HANDLE;
